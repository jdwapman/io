...
# Output is sorted by time, and performance improvements will be in green,
# regressions in red

# We will compare to input file to all JSON files with the same signature
...
# You may want to add more here.
Signatures = ["algorithm", "alpha", "beta", "dataset", "idempotent",
              "source_vertex", "mark_predecessors", "undirected",
              "quick_mode", "num_gpus", "rmat_scale",
              "rmat_edgefactor"]
...
class JSONProc(object):
    ...
    def GatherAll(search_path):
        ...
        print "Found {} JSON files in search path {}".format(len(matches),
                                                             search_path)
        return matches
                ...
                if k == "elapsed":
                    if not math.isinf(last_elapsed):
                        if last_elapsed == 0.0:
                            ...
                            percent = 100 * \
                                (float(v) - last_elapsed) / last_elapsed
                    if percent > 2.0:
...
input_file = filter(
    partial(J.Filter, keywords={"engine": "Gunrock"}), input_file)
all_files = filter(partial(J.Filter, keywords={
                   "engine": "Gunrock"}), all_files)
...
collections = map(lambda s: filter(
    partial(J.Filter, keywords=s), all_files), sig_dedup)
# Extract results for each signature
results = map(lambda c: map(partial(J.Extractor, key=[
              "elapsed", "time", "m_teps"]), c), collections)
# Sort result by ctime
results = map(lambda r: sorted(r, key=lambda k: time.strptime(
    k["time"].replace("\n", ""))), results)
