...
roots = ["../gunrock-output/v1-0-0/bfs"]
fnFilterInputFiles = [
    fileEndsWithJSON,
]
normalize_gpu = "Tesla K40c"
fnPreprocessDF = [
    ...
    normalizeToGPU(
        "speedup",
        "avg-process-time",
        ["dataset", "undirected", "idempotence", "mark-pred", "advance_mode"],
        gpu=normalize_gpu,
    ),
...
chart["full"] = (
    ...
    .mark_point()
    .encode(
        x=alt.X("dataset:N", axis=alt.Axis(title="Dataset",),),
        y=alt.Y(
            "avg-mteps", axis=alt.Axis(title="MTEPS",), scale=alt.Scale(type="log"),
        ),
        color=alt.Color(
            "advance_mode:N",  # brackets allow . in the field name
            legend=alt.Legend(title="Advance Mode",),
        ),
        shape=alt.Shape(
            "undirected_idempotence_markpred:N",
            legend=alt.Legend(title="Undirected / Idempotence / Mark Predecessors",),
        ),
        tooltip=[
            "avg-mteps",
            "avg-process-time",
            "advance_mode, gpuinfo_name:N",
            "undirected",
            "idempotence",
            "mark-pred",
            "tag",
...
chart["lb"] = (
    ...
    .encode(
        ...
        color=alt.Color("advance_mode:N", legend=alt.Legend(title="Advance Mode",),),
        shape=alt.Shape("gpuinfo_name:N", legend=alt.Legend(title="GPU",),),
        tooltip=[
            "avg-mteps",
            "avg-process-time",
            "advance_mode, gpuinfo_name:N",
    ...
    .transform_filter(
        ...
        alt.datum.undirected
        == True & alt.datum.idempotence
        == True & alt.datum["mark-pred"]
        == True
    )
    .interactive()
)
...
chart["lb_v100"] = (
    ...
    .encode(
        ...
        color=alt.Color("advance_mode:N", legend=alt.Legend(title="Advance Mode",),),
        shape=alt.Shape("advance_mode:N", legend=alt.Legend(title="Advance Mode",),),
        tooltip=[
            "avg-mteps",
            "avg-process-time",
            "advance_mode, gpuinfo_name:N",
...
chart["speedup"] = (
    ...
    .encode(
        ...
        y=alt.Y(
            "speedup",
            axis=alt.Axis(title="Speedup over K40c",),
    ...
    save(
        ...
        mdtext=(
         ...
         """
            ...
            + getChartHTML(chart["speedup"], anchor=name)
            + """
