...
def savefile_df(df, name, fileformat):
    # obsolete
    if fileformat == "html":
        open(name + "_data." + fileformat, "w").write(df.to_html())
...
def getChartHTML(chart, anchor=""):
    ...
    chart_html_only = re.search(
        r"<body>\s*(.+)\s*</body>", chart_html.getvalue(), re.DOTALL
    ).group(1)
    # fix the div id now
    chart_html_only = re.sub(
        r'<div id="vis"></div>', r'<div id="vis_%s"></div>' % anchor, chart_html_only
    )
    chart_html_only = re.sub(
        r'vegaEmbed\("#vis", spec, opt\);',
        r'vegaEmbed("#vis_%s", spec, opt);' % anchor,
        chart_html_only,
    )
    # these three patches make the vegaEmbed call use the right div id
    chart_html_only = re.sub(
        r"const el = document.getElementById\('vis'\);",
        r"const el_%s = document.getElementById('vis_%s');" % (anchor, anchor),
        chart_html_only,
    )
    chart_html_only = re.sub(
        r'vegaEmbed\("#vis", spec, embed_opt\)',
        r'vegaEmbed("#vis_%s", spec, embed_opt)' % anchor,
        chart_html_only,
    )
    chart_html_only = re.sub(
        r".catch\(error => showError\(el, error\)\)",
