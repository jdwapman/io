...
do
    ...
    do
        ...
        do
            for l in {0..2}
            do
                echo $EXECUTION ${GRAPH[$d]} --src=${SRC[$s]} $SETTING ${UNDIR[$u]} --device=$DEVICE --traversal-mode=${T_MODE[$l]} --jsondir=./eval/
                $EXEDIR/$EXECUTION ${GRAPH[$d]} --src=${SRC[$s]} $SETTING ${UNDIR[$u]} --device=$DEVICE --traversal-mode=${T_MODE[$l]} --jsondir=./eval/ >> ./eval/${NAME[$d]}.$EXECUTION.output.txt
                sleep 1
            done
        done
    done
