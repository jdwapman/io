...
# General settings
SETTING="--idempotence --queue-sizing=6.5 --in-sizing=4 --iteration-num=10 --direction-optimized"
DEVICE="2"
...
NAME[0]="soc-orkut"           && DO_A[0]="5"          && DO_B[0]="10"
...
do
    ...
    do
        ...
        do
            for l in {0..2}
            do
                echo $EXECUTION ${GRAPH[$d]} --src=${SRC[$s]} $SETTING ${UNDIR[$u]} --device=$DEVICE --traversal-mode=${T_MODE[$l]} --do_a=${DO_A[$d]} --do_b=${DO_B[$d]} --jsondir=./dobfs/
                $EXEDIR/$EXECUTION ${GRAPH[$d]} --src=${SRC[$s]} $SETTING ${UNDIR[$u]} --device=$DEVICE --traversal-mode=${T_MODE[$l]} --do_a=${DO_A[$d]} --do_b=${DO_B[$d]} --jsondir=./dobfs/ >> ./dobfs/${NAME[$d]}.$EXECUTION.output.txt
                sleep 1
            done
        done
    done
